...
export const findSquare = (path: Path, root: Square): Square => {
  ...
  if (path.kind === "nil") { return root; }
  else if (root.kind === "solid") { throw new Error("Not a valid path"); } 
  else if (path.hd === "NW") { return findSquare(path.tl, root.nw); } 
  else if (path.hd === "NE") { return findSquare(path.tl, root.ne); }
  else if (path.hd === "SW") { return findSquare(path.tl, root.sw); }
  else if (path.hd === "SE") { return findSquare(path.tl, root.se); }
